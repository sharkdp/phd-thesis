...
On the other hand, $\gamma(\mathcal{S}) + \gamma(\mathcal{S}^c) = \gamma(T^2) = 2\pi C$, so the Chern number is an integer.
...
Such a condition forces the vector $n_\veck$ to lie in a plane. % TODO
...
Time-reversal symmetry breaking is a necessary requirement for a nonzero Chern number.
