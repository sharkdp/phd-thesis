...
% The main idea is based on a setup with polar molecules that are trapped in a two-dimensional deep optical lattice with quenched tunneling between the sites.
...
% In contrast to non-interacting fermions, free bosons cannot form a topological insulator. However, the bosonic excitations in our system are subject to a hard-core constraint and are expected to give rise to a fractional Chern insulator at $2/3$ filling in $C=2$ topological bands~\cite{Moller2009,Wang2012a,Sterdyniak2015}.
...
\doublefig{.38}{edge/edge-square}{ }{.45}{edge/edge-honeycomb}{ }{Edge states on finite rectangular patches. (a)~Edge state on the square lattice in the $C=2$ phase with opposite orbitals on horizontal and vertical edges. The second edge state has inverted orbitals. (b)~Edge state on the honeycomb lattice between the lower two $C=1$ and $C=0$ bands.}

\doublefig{.38}{edge/edge-square-disordered}{ }{.45}{edge/edge-honeycomb-disordered}{ }{Edge states in the disordered system with a defect density of $\rho=0.2$. (a)~Example on the square lattice for a certain disorder realization. (b)~Edge state on the honeycomb lattice which travels along an interior edge which developed due to the missing lattice sites.}
