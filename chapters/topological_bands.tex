...
Another way to look at the topological properties of the band structure is by means of Berry phases.
Given the Bloch eigenstates $\ket{u_\veck}$ of the lower band with $H_\veck\ket{u_\veck} = E_{\veck,-} \ket{u_\veck}$, we can define the Berry vector potential by
...
\begin{align}
    \gamma = \int_{\partial\mathcal{S}} \mathrm{d}\veck \cdot \vec{A}(\veck) = \int_\mathcal{S} \mathrm{d}^2 \, \veck B(\veck)
\end{align}
...
\end{align}
In this light, the Chern number can be seen as the total Berry curvature in the Brillouin zone divided by $2\pi$.
