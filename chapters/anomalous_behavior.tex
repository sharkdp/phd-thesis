...
\section{Realizing the dipolar spin Hamiltonian}
...
\epsZ - 2 \pi a |\vecq|$ for small values $q \ll 1/a $, while all
...
anti-ferromagnetic XY model) are presented in \cref{spinwave_analysis}. The results are
...
\begin{figure}[ht]
    \centering
    \ig{.99}{fig2}
    \caption{Spin wave excitations with $\Gamma=(0,0)$, $M=(0,\pi/2)$, and $K=(\pi/a,\pi/a)$ for different $\theta$ angles. (a) Spectrum of the I-F phase which also shows the behavior of the dipolar dispersion $\epsq$ for $\theta=-3\pi/4$, see red line. (b-d) Spectrum for the XY-F, I-AF and XY-AF phases. Each red line is a critical excitation spectrum indicating an instability.\tlabel{fig2}}
\end{figure}
...
turn, for anti-ferromagnetic XY couplings, the gap is minimal at $\vecK$,
...
\sin \theta\, |{\vecq}|$. Consequently, the dynamics of spin waves at low momenta is in analogy to the
...
$\langle (-1)^{i-j}S_{i}^{z}S_{j}^{z} \rangle \sim 1/|{\vecr}|^3$ for the anti-ferromagnetic transverse spin correlation at zero temperature.
