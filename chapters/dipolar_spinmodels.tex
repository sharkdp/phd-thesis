...
\subsection{Two-level dipoles}
%TODO
%Holstein - Primakoff
Take the spin model from \tref{xyfull}
\begin{align}
    H &= \frac{1}{2} \sum_{i\ne j} \frac{\kappa}{R_{ij}^3} \bc{ \frac{(d_1-d_0)^2}{4} \sz_i \sz_j  + q_1^2 (\splus_i \sminus_j + \sminus_i \splus_j) }
\end{align}
and introduce the HC operators $\bopd_i=\sigma^+_i$, $\bop_i=\sigma^-_i$. They fulfill
Then,
\begin{align}
    \Hdd_{ij} &= \frac{\kappa}{R_{ij}^3} \Big[ \frac{(d_1-d_0)^2}{4} \sz_i \sz_j  + q_1^2 (\splus_i \sminus_j + \sminus_i \splus_j)
\end{align}
