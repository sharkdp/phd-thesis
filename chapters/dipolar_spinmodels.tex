...
For most applications, we will be concerned with two-dimensional systems where the dipoles are aligned perpendicular to the plane. Then, the dipoles are also perpendicular to the interconnecting axis $\vecR_{ij}$, implying $\theta = \pi/2$. In this case, the tensorial part reduces to
\begin{align}
...
\end{align}
Converseley, if the dipoles point along the lattice direction, we can set $\theta=0$ in \tref{doperator} to get
\begin{align}
