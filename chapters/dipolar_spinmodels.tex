...
\subsection{Two-level dipoles: Angular momentum difference}
\tlabel{twoleveldm}
...
Including all states in \tref{dipole-moments} and a (possibly) tilted external field, we find
\begin{align}
    t_{ij} &= \frac{\kappa}{R_{ij}^3} \begin{pmatrix}
...
In the presence of the static electric field, we denote the lowest rotational states having $m=0, \pm 1$ by $\ketz$ and $\ketpm$, respectively.
...
\end{align}
For $\Theta=0$, the interaction reduces to the expression given in \tref{ddint}.
