...
same $M$ quantum number.
Then, only the $d^0_id^0_j$ part of the dipolar interaction is relevant.
We define the (transition) dipole elements
\begin{align*}
    d_{0} = \smallbraketop{0}{d^0}{0},\qquad
    d_{1} = \smallbraketop{1}{d^0}{1},\qquad
    d_{01} = |\smallbraketop{0}{d^0}{1}| = |\smallbraketop{1}{d^0}{0}|.
\end{align*}
...
\begin{align*}
    \Hdd_{ij} &= \frac{\kappa}{R_{ij}^3} \bc {d_{1}^2 P^1_i P^1_j + d_{0}^2 P^0_i P^0_j + d_{1}d_{0} (P^1_i P^0_j + P^0_i P^1_j) + d_{01}^2 (\splus_i \sminus_j + \sminus_i \splus_j) }.
\end{align*}
...
\begin{align*}
    \Hdd_{ij} &= \frac{\kappa}{R_{ij}^3} \Big[ \frac{(d_1-d_0)^2}{4} \sz_i \sz_j  + d_{01}^2 (\splus_i \sminus_j + \sminus_i \splus_j)\\
...
We proceed similar as before and denote the two states by $\ketz$ and $\ketp$, indicating the difference of $\Delta M = 1$ between the two states. The relevant dipole matrix elements are\footnote{The sign difference in the last term is due to $\smallbraketop{0}{d^-}{+} = \smallbraketop{+}{(d^-)^\dag}{0}^* = \smallbraketop{+}{- d^+}{0}^*$. Note that we can choose the phases of $\ketz$ and $\ketp$ freely, allowing us to choose real values for the transition dipole elements.}
\begin{align}
    d_0 = \smallbraketop{0}{d^0}{0},\qquad
    d_+ = \smallbraketop{+}{d^0}{+},\qquad
    d_{+0} = |\smallbraketop{+}{d^+}{0}| = -d_{0+}.
\end{align}
...
\begin{align*}
    \Hdd_{ij} &= \frac{\kappa}{R_{ij}^3} \Big[ \frac{(d_+-d_0)^2}{4} \sz_i \sz_j  - \frac{d_{0+}^2}{2} (\splus_i \sminus_j + \sminus_i \splus_j) \\
...
\begin{align}
    d_0 &= \smallbraketop{0}{d^z}{0} = -\mu_\text{B}g/2, \\
    d_+ &= \smallbraketop{+}{d^z}{+} = +\mu_\text{B}g/2, \\
    d_{+0} &= |\smallbraketop{+}{d^+}{0}| = \smallbraketop{+}{\sqrt{2} \sigma^+}{0} = +\mu_\text{B}g/\sqrt{2}.
\end{align}
...
\subsection{Three-level dipoles with \texorpdfstring{$\Delta M_{1,2} = \pm 1$}{dM = -1, +1}}
same as before. $d^0d^0$ leads to interaction terms. $d^+d^-$ provides tunneling for both $+$ and $-$. But now there is an energy conserving $d^+d^+$ contribution, the $+$ to $-$ tunneling.


\subsection{Three-level dipoles with \texorpdfstring{$\Delta M_{1,2} = 0, 1$}{dM = 0, +1}}
same as before but we now need a tilted field to enable the $d^+d^0$ terms.
