...
\end{align}
As before, we define hardcore bosonic operators $\bop_\pm=\ketbra{0}{{\pm}}$ and $\nop_{\pm}=\bopd_{\pm}\bop_{\pm}$, and write the dipole operator in terms of these:
\begin{align}
    d^0 &= d_+ \nop_+ + d_- \nop_- + d_0 (1-\nop_+-\nop_-) = d_0 + (d_+-d_0) \nop_+ + (d_--d_0) \nop_-, \\
    d^+ &= q_+ \bopd_+ - q_- \bop_-, \\
    d^- &= - q_+ \bop_+  + q_- \bopd_-.
\end{align}
using this, we can write the relevant parts of the dipole-dipole interaction as
\begin{align}
...
\end{align}
where we have neglected any excitation-nonconserving terms. Further dropping the constant terms and \qu{magnetic field} terms, we find:
\begin{align}
    H = \frac{1}{2} \sum_{i\ne j} \Hdd_{ij} = \sum_{i\ne j}  t_{ij}^{\alpha\beta} \bopd_{\alpha,i} \bop_{\beta,j}
    +\frac{1}{2}\sum_{i\ne j} V_{ij}^{\alpha\beta} \nop_{\alpha,i} \nop_{\beta,j}
\end{align}
where summation over the $\alpha, \beta$ indices, labelling the internal state of the excitation, is assumed. The tunneling rates and interaction matrix elements are given by
\begin{align}
...
\end{align}
Introducing the lattice spacing $a$, we can define the nearest-neighbor tunneling rates
...
\end{align}
This model is the basis for the realization of topological band structures in \cref{topological_bands}.
