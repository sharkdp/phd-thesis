...
\sidefig{.45\textwidth}{epsk/epsk}{Plot of the function $\epskm$ on a high-symmetry path in the Brillouin zone of the square lattice. The linear behavior at $\veck=\vecGamma$ is apparent as well as the zeros of $\epsk^2$ at $\veck=\vecGamma, \vecM$.}
...
$\phi \approx \phi_\vecR$ is the angle between $\vecR$ and the $x$-axis and $\psi$ is the
corresponding angle between $\veck$ and the $k_x$-axis. Then
\begin{align}
...
we cannot determine ($J_m$ is the Bessel function of the first kind). Consequently, this calculation is only useful
if we find quantities like $\epskm-\epsZ^m$ which (a) allow
us to safely take the limit $z_c = k R_c \goesto 0$ while keeping $R_c$ constant and
...
Setting $s=3 \Leftrightarrow m=0$ in \tref{twosum}, we find:
\begin{align}
