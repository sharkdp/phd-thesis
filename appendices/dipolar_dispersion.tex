...
\fig{.5}{epsk/epsk}{epsk}{Plot of the function $\epskm$ on a high-symmetry path in the Brillouin zone of the square lattice. The linear behavior at $\veck=\Gamma$ is apparent as well as the zeros of $\epsk^2$ at $\veck=\Gamma, M$.}
...
\tlabel{lowmomentum}
For small $|\veck| \ll 1$ we can make a crude approximation\footnote{We will re-derive the results in this section in a \qu{cleaner} way later.} and replace the discrete Fourier series
...
\subsection{Nonanalytic behavior at low momenta}
...
\end{align}
For $m=0$ the prefactor is given by $-2\pi$, see \tref{epsqewald}, and for $m=2$ we find $2\pi/3$. Both results are in accordance with the the estimation in \tref{lowmomentum}.
